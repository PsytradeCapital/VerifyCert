...
/**
 * SYSTEMATIC SYNTAX FIX - PRESERVE ALL FUNCTIONALITY
 * This will fix syntax errors while keeping all features intact
 */

const fs = require('fs');
const path = require('path');

console.log('🔧 SYSTEMATIC SYNTAX FIX - PRESERVING ALL FUNCTIONALITY');
console.log('========================================================');

// Fix CertificateViewer.tsx - the immediate issue
function fixCertificateViewer() {
  console.log('🔨 Fixing CertificateViewer.tsx...');
  
  const fixedCertificateViewer = `import React, { useState, useEffect, useCallback } from 'react';
...
export default function CertificateViewer(): JSX.Element {
  ...
  try {
    fs.writeFileSync('frontend/src/pages/CertificateViewer.tsx', fixedCertificateViewer);
    console.log('✅ CertificateViewer.tsx fixed');
    return true;
  } catch (error) {
    console.error('❌ Failed to fix CertificateViewer.tsx:', error.message);
    return false;
  }
}

// Fix IssuerDashboard.tsx - preserve all demo functionality
function fixIssuerDashboard() {
  console.log('🔨 Ensuring IssuerDashboard.tsx is properly structured...');
  
  try {
    const content = fs.readFileSync('frontend/src/pages/IssuerDashboard.tsx', 'utf8');
    
    // Check if it has proper export
    if (!content.includes('export default function IssuerDashboard')) {
      console.log('⚠️ IssuerDashboard needs export fix');
      
      // Add proper export if missing
      const fixedContent = content.replace(
        /function IssuerDashboard\(\)/g,
        'export default function IssuerDashboard()'
      );
      
      fs.writeFileSync('frontend/src/pages/IssuerDashboard.tsx', fixedContent);
    }
    
    console.log('✅ IssuerDashboard.tsx structure verified');
    return true;
  } catch (error) {
    console.error('❌ Failed to fix IssuerDashboard.tsx:', error.message);
    return false;
  }
}
...
async function runFixes() {
  console.log('Starting systematic fixes...\n');
  
  const results = [];
  
  results.push(fixCertificateViewer());
  results.push(fixIssuerDashboard());
  
  const successCount = results.filter(Boolean).length;
  const totalCount = results.length;
  
  console.log(`\n📊 RESULTS: ${successCount}/${totalCount} fixes successful`);
  ...
  if (successCount === totalCount) {
    ...
    console.log('3. All functionality should be preserved');
  } else {
